            ...
            <tbody className="bg-white">
              {pembelian?.map((item) => (
                <tr key={item.id} className="w-full border-b py-3 text-sm last-of-type:border-none">
                  <td className="px-4 py-5">{item.nama_pegawai}</td>
                  <td className="px-4 py-5">{item.nama_distributor}</td>
                  <td className="px-4 py-5">{formatDateToLocal(item.date)}</td>
                  <td className="px-4 py-5">{item.jumlah}</td>
                  <td className="px-4 py-5">{formatCurrency(item.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
